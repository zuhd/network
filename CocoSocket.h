...
unsigned char key1[16] = { 117, 42, 81, 222, 211, 85, 189, 17, 
		92, 159, 6, 102, 179, 2, 100, 0 };

unsigned char key2[16] = { 127, 31, 79, 202, 251, 81, 179, 67, 
...
private:
	unsigned char	m_szSymmKey[16];	// 对称密钥
...
public:
	...
	virtual void onMessage(const char* msg,int len) = 0;
...
class CocoSocket
{
	...
	int m_fd;
	bool m_bConnected;
	CBuffer<2048> m_ReadBuffer;
	CBuffer<2048> m_WriteBuffer;
...
int CNetPacketParser::EncodePacket(const char* pInData, int nInLen, char* pOutData, int& nOutLen)
{
	...
	else
	{
		...
		{
			cCRC += (pData[i] << 2);
			pData[i] = (i % 3 == 1) ? ((pData[i] << 2) | (pData[i] >> 6)) : ((pData[i] << 5) | (pData[i] >> 3));
		}
		cCRC ^= m_szSymmKey[12];
		unsigned char* pTable = (unsigned char*)&table[cCRC*4];
		int k = 0;
		for(int i = 0; i < nPacketLen; ++i)
		{
			pData[i] ^= pTable[k];
...
int CNetPacketParser::DecodePacket(const char* pInData, int nInLen, char* pOutData, int& nMaxLen)
{
	...
	else
	{
		if(cCompress != 1)
		{
			...
			for(int i = 0; i < usPacketLen; ++i)
			{
				pData[i] = (i % 3 == 1) ? ((pData[i] << 6) | (pData[i] >> 2)) :	((pData[i] << 3) | (pData[i] >> 5));
				cValiate += (pData[i] << 2);
			}
...
bool CocoSocket::connect(const char* ip, unsigned short port, float timeout)
{
	...
	int retval = ::connect(m_fd, (struct sockaddr*)&svraddr, sizeof(svraddr));
	if (retval == 0)
	{
		onConnectedEvent();
	}
	...
	{
		CCLOG("connect error. errno:%d", GetLastError());
		if (m_pSessionHandler)
			m_pSessionHandler->onConnectFailed();
		return false;
	}
	//add to reactor.
...
bool CocoSocket::onReadEvent()
{
	...
	if (retval > 0)
	{
		...
		//after dispatch then shrink the buffer
...
bool CocoSocket::onWriteEvent()
{
	...
	if (used > 0)
	{
		...
		int nPacketLen = MAX_MSG_LEN;
		m_pPacketParser.EncodePacket(base, used, pPacket, nPacketLen);
		retval = send(m_fd, pPacket, nPacketLen, 0);
		if (retval == -1)
		{
			int err = GetLastError();
			if (err == ERR_EAGAIN || err == ERR_EINTR)
...
void CocoSocket::onMessageEvent(const char* msg, int len)
{
	if (m_pSessionHandler)
	{
		m_pSessionHandler->onMessage(msg, len);
	}
}
void CocoSocket::onDisconnectedEvent()
{
	if (m_pSessionHandler)
	{
		m_pSessionHandler->onDisconnected();
	}
}
	...
	while(true)
	{
		...
		if(nParserLen > 0)
		{
			...
			if(nMaxPacket > 0 && nMaxPacket <= MAX_MSG_LEN) //取到完整包, 进队列
			{
				if (m_pSessionHandler)
				{
					m_pSessionHandler->onMessage(pPacket, nMaxPacket);
				}			
			}	
			// 如果还有剩余的长度，证明
			// 有多个包，进入下个循环
...
void CocoSocketReactor::handleEvents()
{
	...
	while (it != m_ConnectorList.end())
	{
		...
		if (FD_ISSET(tor->fd(), &m_ErrorFDs))
		{
			...
			{
				CCLOG("can't connect to %s:%d\n", tor->m_szIp, tor->m_usPort);
				tor->onConnectFailedEvent();
			}
			else
			{
				CCLOG("unkown error. disconnect");
...
int main()
{
	g_pASConnector = new CocoSocket();
	g_pASConnector.Connect(192.168.17.1);
	CocoSocketReactor::getInstance()->addSocket(g_pASConnector);
}
